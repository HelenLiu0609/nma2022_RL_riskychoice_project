...
df_new <- df %>% dplyr::select(
  ...
  "TrialType", #three types of trials
  "StimType", #stimulus
  "Block", #6 blocks
  "trial_index" = "Trial", #trial number
  "response" = "Choice", #participants' choice
  "RewardAmount", #amount of reward participants gets
  "CumulativeReward" #the cumulated reward participants gets
) %>%
  dplyr::filter(TrialType != "NULL") %>% #exclude the breaks
  dplyr::mutate(
    n_Block = case_when( #correct the block numbers
      Block == "1" ~ "1",
      Block == "3" ~ "2",
      Block == "5" ~ "3",
      Block == "7" ~ "4",
  ...
  dplyr::mutate(
    TypeTrial = case_when( #mark types of trials 
    TrialType == "" ~ "forced_choice", # only one door
    TrialType == "Decision" ~ "Decision Trial", #chose between the safe and risky options with equal expected values 12/34
  ...
  dplyr::mutate(
    expected_response = case_when( #mark expected response (maximize reward)
    StimType == "102"|StimType == "201" ~ "Door2", #decision
    StimType == "103"|StimType == "301" ~ "Door3", #catch
    StimType == "104"|StimType == "401" ~ "Door4",#catch
    StimType == "203"|StimType == "302" ~ "Door3", #catch
    StimType == "204"|StimType == "402" ~ "Door4", #catch
...
#save stats
#path_out = '~/Desktop/nma2022_project_RL_riskychoice/data/'#save file
#fileName = paste(path_out, 'stats_human.csv', sep = '')
#write_csv(stats, fileName, col_names = TRUE, append = FALSE)
```
