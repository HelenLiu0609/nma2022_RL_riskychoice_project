...
```{r import data}
df_sim <- read.csv("~/Desktop/nma2022_project_RL_riskychoice/data/EpsilonGreedyData_28agents.csv")
#str(df)
head(df_sim)
#check data 
xxx <- df_sim %>%
...
```{r import data}
xxx <- df_sim_clean %>% filter(TrialType == "decision")%>%
...
tiff(
  file = "descript_ginger_human.tiff",
  res = 800,
  width = 4500,
  height = 3500
)#save tiff
